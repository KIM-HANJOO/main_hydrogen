...
class model() :
    ...
    def __init__(self, main_dir) :
        pre_dir = os.path.join(main_dir, '전체 업태')
        ...
        for folder in os.listdir(pre_dir) :
            if os.path.isdir(os.path.join(pre_dir, folder)
                folder_list.append(folder)
        print(folder_list)
        
        for num, folder in enumerate(folder_list) :
                        temp_dir = os.path.join(model_dir, folder)
            try :
            ...
            for model_name in model_list :
                if 'model4' not in model_name :
                    if 'plot' not in model_name :
                        for de in dayends :
                            adddir = os.path.join(model_dir, folder, model_name, de)
                            newfolder(adddir)
        
        for folder in os.listdir(pre_dir) :
            tempdir = os.path.join(pre_dir, folder)
            weekend_src = os.path.join(tempdir, '7_봄가을', '주말')
            weekday_src = os.path.join(tempdir, '7_봄가을', '주중')
            
            weekend_dst = os.path.join(model_dir, folder, 'preprocessed', '주말')
            weekday_dst = os.path.join(model_dir, folder, 'preprocessed', '주중')
            
            for f in os.listdir(weekend_src) :
                shutil.copyfile(os.path.join(weekend_src, f), os.path.join(weekend_dst, f))
            for f in os.listdir(weekday_src) :
                shutil.copyfile(os.path.join(weekday_src, f), os.path.join(weekday_dst, f))
    ...
    def reset(self) :
        for everything in os.listdir(self.model_dir) :
            remove(os.path.join(self.model_dir, everything))
...
# MODEL 3
    ...
    def model3_unit(self, finaldir, model3dir) :
        ...
        osdays = os.path.join(finaldir, '주중')
        osends = os.path.join(finaldir, '주말')
        
        for de in folderlist :
            osnow = os.path.join(finaldir, de)
            if de == folderdays :
                osnow = osdays
            else :
                osnow = osends
            ...
            for excel in os.listdir(osnow) :
                ...
                os.chdir(os.path.join(model3dir, de))
        ...
        for folder in os.listdir(model3dir) :
            tempdir = os.path.join(model3dir, folder)
            if '주중' in folder :
                folderdays = folder
                model3days = tempdir
            elif '주말' in folder :
    ...
    def model3(self) :
        ...
        for condition in self.condition_list :
            ...
            srcdir = os.path.join(self.src_dir, condition)
            cwdir = os.path.join(self.model_dir, condition)
            finaldir = os.path.join(cwdir, 'preprocessed')
            model3dir = os.path.join(cwdir, 'model3')
            model3cluster = os.path.join(cwdir, 'model3_cluster')
            model4dir = os.path.join(cwdir, 'model4') 
