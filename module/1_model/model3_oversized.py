...
for group in range(4) :
    ...
    df_0 = df_now[df_now['group'] == group * 2 + 0]
    df_1 = df_now[df_now['group'] == group * 2 + 1]

    df_0.reset_index(drop = True, inplace = True)
    df_1.reset_index(drop = True, inplace = True)

    profile_all = df_now.loc[:, 'hours_1' : 'hours_48'].mean().tolist()
    profile_0 = df_0.loc[:, 'hours_1' : 'hours_48'].mean().tolist()
    profile_1 = df_1.loc[:, 'hours_1' : 'hours_48'].mean().tolist()

    group_0 = []
    group_1 = []

    for hour in range(48) :
        group_0.append(profile_0[hour] - profile_all[hour])
        group_1.append(profile_1[hour] - profile_all[hour])

    ax = None
    ax = fig.add_subplot(2, 2, group + 1)

    ax.bar(xvalues, group_0, color = 'blue', alpha = 1, label = 'group_0 - representative')
    ax.bar(xvalues, group_1, color = 'red', alpha = 1, label = 'group_1 - representative')
...
plt.legend()
dlt.savefig(profile_dir, 'watch.png', 400)
