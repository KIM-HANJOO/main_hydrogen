    ...
    for i in range(profile_num):
        ...
        for col in model2_file.columns :
            if facility in col :
                if '주중' in col :
                    print(col)
                    c = model2_file.loc['c', col]
                    d = model2_file.loc['d', col]
                    loc = model2_file.loc['loc', col]
                    scale = model2_file.loc['scale', col]
        
        st_week = burr.rvs(c, d, loc = loc, scale = scale, size = 1)
        ...
        st_weekend = burr.rvs(c, d, loc = loc, scale = scale, size = 1)
...
def generate_fc(facility, group) :
    ...
    # main
        m1 = os.path.join(facility_dir, 'model1')
        m2 = os.path.join(facility_dir, 'model2')
        m3 = os.path.join(facility_dir, 'model3_cluster')
        m4 = os.path.join(facility_dir, 'model4')
